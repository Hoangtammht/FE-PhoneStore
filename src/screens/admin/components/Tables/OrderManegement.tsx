        ...
        <tbody>
          {filteredOrders.map((order) => (
            <tr key={order.orderID}>
              ...
              <td className="p-2 border">{order.priceAtOrder.toLocaleString()} VND</td>
              <td className="p-2 border">{order.orderType === 'installment' ? 'Trả góp' : 'Thường'}</td>
              <td className="p-2 border">{order.content || ''}</td>
              <td className="p-2 border">{order.durationMonths}</td>
              <td className="p-2 border">{order.monthlyPayment}</td>
